...
/** List adapter for generic types, intended use for small-medium lists of data. */
class GenericListAdapter<T>(
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int) = GenericListViewHolder(
        when {
            ...
            else -> throw IllegalStateException("Either the layout ID or the view factory need to be non-null")
        }
    )
